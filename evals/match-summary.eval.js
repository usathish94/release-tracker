// Run: npm run eval
//
// Regression check for the match-summary skill (src/skills/match-summary):
// runs it against the fixed dataset in sample-dataset.jsonl and grades every
// output two ways - deterministic rule checks for what code can verify
// exactly, and an LLM-as-judge pass (schema-constrained) for what only a
// reader can judge. Wire this into CI so a prompt edit that regresses
// quality fails the build instead of shipping silently.
...
function ruleBasedChecks(input, output) {
  ...
  if (sentenceCount !== 2) failures.push(`expected 2 sentences, counted ${sentenceCount}`);
  if (
    input.score == null &&
...
for (const { input } of dataset) {
  ...
  if (!judged.sounds_energetic || !judged.grounded_in_input) console.log(`  judge: ${judged.reasoning}`);
}
